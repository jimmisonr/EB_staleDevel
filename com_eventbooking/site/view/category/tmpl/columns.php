...
<div id="eb-category-page-columns" class="eb-container">
	<form method="post" name="adminForm" id="adminForm" action="<?php echo JRoute::_('index.php?option=com_eventbooking&view=category&layout=default&id='.$this->category->id.'&Itemid='.$this->Itemid); ?>">
		<?php
			...
			if (count($this->items))
			{
				echo EventbookingHelperHtml::loadCommonLayout('common/tmpl/events_columns.php', array('events' => $this->items, 'config' => $this->config, 'Itemid' => $this->Itemid, 'nullDate' => $this->nullDate , 'ssl' => $ssl, 'viewLevels' => $this->viewLevels, 'category' => $this->category, 'Itemid' => $this->Itemid, 'bootstrapHelper' => $this->bootstrapHelper));
			}
			else
			{
